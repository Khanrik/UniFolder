...
let foldl =
  ...
  let rec foldl f a t =
    ...
    | Branch (_,_,l,r) -> foldl f (foldl f a l) r in
  foldl
...
let foldr =
  ...
  let rec foldr f t a =
    match t with
      Empty -> a
    | Leaf(_,l) -> foldr_list f l a
    | Branch(_,_,l,r) -> foldr f l (foldr f r a) in
  foldr
...
let dom f = setify (foldl (fun a x _ -> x::a) [] f)

let ran f = setify (foldl (fun a _ y -> y::a) [] f)
...
let defined f x = try let _ = apply f x in true with Failure _ -> false
...
let equate (a,b) (Partition f as ptn) =
  ...
  Partition
   (if a' = b' then f else
    if na <= nb then
       List.fold_right (fun x -> x) [a' |-> Nonterminal b'; b' |-> Terminal (b',na+nb)] f
    else
       List.fold_right (fun x -> x) [b' |-> Nonterminal a'; a' |-> Terminal (a',na+nb)] f)
