...
let int_func f args = 
  ...
  | ("*",[Con (x, []);Con (y, [])]) -> Con (string_of_int (int_of_string x * int_of_string y), [])
  | ("-",[Con (x, []);Con (y, [])]) -> Con (string_of_int (int_of_string x - int_of_string y), [])
  | ("=",[Con (x, []);Con (y, [])]) -> Con ((if x = y then "1" else "0"), [])
  | ("?",[Con (x, []);y;z]) -> if x = "1" then y else z
  | _ -> Con (f, args)

let int_func_lazy e f args = 
  match f with
    "+" | "*" | "-" | "=" -> int_func f (map e args)
  | "?" -> (
    match args with 
      [x;y;z] -> (
        match e x with 
...
let eval_int s = eval int_func (lparser s)

let eval_int_lazy s = evalazy int_func_lazy (lparser s)
...
let print_lambda formatter =
  let rec print_lambda pr fm =
    ...
    | App(p,q) -> bracket formatter (pr > 1) 1 (print_infix 1 "@") p q
